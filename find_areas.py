...
def compare_tf_idf(tf_idf_text, knowledge_areas):
    similarities = []
    for area in knowledge_areas:
        ...
        for term in tf_idf_text:
            area_vector.append(area["term_frequencies"].get(term, 0))
            text_vector.append(tf_idf_text[term])
        
        for term in area["term_frequencies"]:
            if term not in tf_idf_text:
                area_vector.append(area["term_frequencies"][term])
                text_vector.append(0)
        
        similarity = cosine_similarity(np.array(text_vector), np.array(area_vector))
        code = area.get("code", " ")
        #if similarity >= 0.00:            
        similarities.append((code, area["name"], similarity))

    similarities.sort(key=lambda x: x[2], reverse=True)
    top_similar_areas = similarities
    return top_similar_areas

def read_file(file_path):
        with open(file_path, 'r', encoding='utf-8') as file:
            return file.readlines()

def process_courses(file_path_a_general, file_path_a_field, file_path_b_core, knowledge_areas):
    ...
    for course in courses:
        ...
        similar_areas = compare_tf_idf(tf_idf_text, knowledge_areas)
        results.append({
            "name": course["name"],
            "kp": course["kp"],
            "type": course["type"],
            "code": course["code"],
            "similarities": similar_areas
        })
...
tf_idf_file_path = f'Guidelines/{paths[subcategory]}/tf_idf_area.json'
...
output_file_path = f'Guidelines/{paths[subcategory]}/results/similar_areas.json'

knowledge_areas = load_json(tf_idf_file_path)

results = process_courses(a_core_general_file_path, a_core_field_file_path, b_core_curriculum_file_path, knowledge_areas)
